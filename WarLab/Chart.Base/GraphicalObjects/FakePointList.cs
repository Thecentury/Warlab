...
namespace ScientificStudio.Charting.GraphicalObjects {
	internal sealed class FakePointList : IList<Point> {
		...
		internal FakePointList(List<Point> points)
			: this(points, Double.NegativeInfinity, Double.PositiveInfinity) { }
		...
		internal void SetBorders(double left, double right) {
			this.left = left;
			this.right = right;
			Calc();
		}
		...
		private void Calc() {
			...
			first = points.FindIndex(p => p.X > left);
			if (first > 0)
				first--;

			last = points.FindLastIndex(p => p.X < right);
